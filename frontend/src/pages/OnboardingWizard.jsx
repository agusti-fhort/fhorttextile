...
export default function OnboardingWizard() {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: 600, margin: '0 auto' }}>
      <div style={{ marginBottom: 8, fontSize: 'var(--fs-label)', color: 'var(--text-muted)', letterSpacing: '.08em', textTransform: 'uppercase' }}>
        FHORT Textile Tech · Onboarding
      </div>
      <h1 style={{ fontSize: 'var(--fs-h1)', fontWeight: 500, color: 'var(--gold)', margin: '0 0 24px' }}>
        Configuració inicial
      </h1>

      {msg && (
        <div style={{ padding: '8px 12px', marginBottom: 16, borderRadius: 4, fontSize: 'var(--fs-body)',
          ...
          {status && (
            <div style={{ padding: '12px 16px', border: '1px solid var(--border)', borderRadius: 6, marginBottom: 20 }}>
              ...
              {Object.entries(status.steps || {}).map(([k, s]) => (
                <div key={k} style={{ display: 'flex', gap: 8, padding: '3px 0', fontSize: 'var(--fs-body)' }}>
                  <span style={{ color: s.ok ? '#3b6d11' : 'var(--text-muted)' }}>{s.ok ? '✓' : '○'}</span>
                  <span style={{ color: s.ok ? 'var(--text-main)' : 'var(--text-muted)' }}>{s.label}</span>
                  <span style={{ marginLeft: 'auto', color: 'var(--text-muted)', fontSize: 'var(--fs-label)' }}>{s.descripcio}</span>
                </div>
              ))}
            </div>
          )}
          <button onClick={() => setStep(1)} style={{
            padding: '9px 20px', borderRadius: 4, fontSize: 'var(--fs-body)', cursor: 'pointer',
      ...
      {step === 1 && (
        <div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 'var(--fs-body)', color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Nom de l\'empresa *</label>
            <input value={config.nom_empresa} onChange={e => setConfig(c => ({...c, nom_empresa: e.target.value}))}
              placeholder="Ex: Textiles Brownie SL"
              style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: 4, fontSize: 'var(--fs-body)', boxSizing: 'border-box' }} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 'var(--fs-body)', color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Unitats de mesura</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {['CM', 'INCH'].map(u => (
                <button key={u} onClick={() => setConfig(c => ({...c, unitat_mesura: u}))} style={{
                  padding: '6px 16px', borderRadius: 4, fontSize: 'var(--fs-body)', cursor: 'pointer',
      ...
      {step === 2 && (
        <div>
          <p style={{ fontSize: 'var(--fs-body)', color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.6 }}>
      ...
      {step === 3 && (
        <div>
          <div style={{ fontSize: 'var(--fs-body)', fontWeight: 600, color: '#3b6d11', marginBottom: 16 }}>
            ✓ Configuració completada!
          </div>
          {uploadResult && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 'var(--fs-body)', color: 'var(--text-muted)', marginBottom: 8 }}>Dades carregades:</div>
