...
export default function FileList({ files, selectedId, onSelect, onOpen, emptyLabel }) {
  ...
  const capçalera = (col) => (
    <button key={col} type="button" onClick={() => setSort(s => ({ col, asc: s.col === col ? !s.asc : true }))}
      style={{
        ...
        color: sort.col === col ? 'var(--gold)' : 'var(--text-soft)', padding: 0,
  ...
  if (!files) {
    return <div style={{ padding: 16, color: 'var(--text-soft)', fontSize: 'var(--fs-body)' }}>{t('app.loading')}</div>
  }
  if (!files.length) {
    return (
      <div style={{ padding: 24, textAlign: 'center', color: 'var(--text-soft)', fontSize: 'var(--fs-body)', fontStyle: 'italic' }}>
  ...
  return (
    <div>
      <div style={{
        display: 'flex', gap: 14, padding: '6px 10px', borderBottom: '1px solid var(--line)',
            ...
            <li key={f.id}>
              <button type="button"
                ...
                <i className={`ti ${iconaDe(f.nom_fitxer)}`} aria-hidden="true"
                  style={{ fontSize: 16, color: actiu ? 'var(--gold)' : 'var(--text-soft)', flexShrink: 0 }} />
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {f.derivat_de_label && (
                    <span style={{ display: 'block', fontSize: 'var(--fs-caption)', color: 'var(--text-soft)', marginTop: 1 }}>
                ...
                <span style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-soft)', width: 90, flexShrink: 0 }}>{f.tipus}</span>
                <span style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-soft)', width: 56, flexShrink: 0, textAlign: 'right' }}>{midaLlegible(f.mida_bytes)}</span>
                <span style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-soft)', width: 68, flexShrink: 0, textAlign: 'right' }}>{dataCurta(f.data_pujada)}</span>
                <span style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-soft)', width: 34, flexShrink: 0, textAlign: 'right' }}>v{f.versio}</span>
                <span style={{
                  fontSize: 'var(--fs-caption)', color: 'var(--text-soft)', width: 96, flexShrink: 0,
