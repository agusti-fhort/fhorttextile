...
def build_extraction_prompt(wizard_context=None) -> str:
    ...
    return (
        ...
        "\n"
        "## HEADER FIELDS — style_reference (codi) vs style_name (nom_prenda)\n"
        "\n"
        "Treat these as two separate fields and never confuse them:\n"
        "\n"
        "STYLE_REFERENCE (also fill `style_code` with the same value):\n"
        "- Capture the short alphanumeric client code that identifies the document.\n"
        "- Look for any of these labels in the header or title block:\n"
        "  \"Style No:\", \"Style:\", \"Ref:\", \"Reference:\", \"Style Name:\",\n"
        "  \"Art.\", \"Artículo:\", \"Referencia:\", \"Ref. client:\", \"Code:\".\n"
        "- If more than one candidate is present, prefer the short alphanumeric\n"
        "  code (e.g. \"26-08A\") over a long descriptive title\n"
        "  (e.g. \"REPRISE SUMMER\").\n"
        "- If no explicit reference label is found, return `style_reference`\n"
        "  (and `style_code`) as null. Do NOT invent one.\n"
        "\n"
        "STYLE_NAME (mapped to `nom_prenda` in the database):\n"
        "- Capture the stylistic / descriptive name of the garment\n"
        "  (e.g. \"Olivia Dress\", \"Top 2\", \"Pantalon Chino\").\n"
        "- This is NOT the reference code. Never copy the alphanumeric reference\n"
        "  into `style_name`.\n"
        "- If the document title contains both, split them:\n"
        "    style_name      → the descriptive name\n"
        "    style_reference → the alphanumeric code\n"
        "- If no descriptive name is present, return `style_name` as null.\n"
        "\n"
    )
